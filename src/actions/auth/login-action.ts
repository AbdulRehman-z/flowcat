...
import { get2FAConfirmationByUserId } from "@/data/auth/two-factor-cofirmation";
...
export const loginAction = async (formData: z.infer<typeof loginSchema>) => {
  ...
  if (existingUser.isTwoFactorEnabled && existingUser.email) {
    if (code) {
      ...
      await db.delete(twoFactorTokens).where(eq(twoFactorTokens.id, existingTwoFactorToken.id!))

      const existing2FACofirmation = await get2FAConfirmationByUserId(existingUser.id)
      if (existing2FACofirmation) {
        await db.delete(twoFactorConfirmations).where(eq(twoFactorConfirmations.id, existing2FACofirmation.id!))
      }
      ...
      const generated2FAToken = await generate2FactorToken(existingUser.email)
