...
export const GetCommunityPromptsCategory = async () => {
  try {
    ...
    const result = await db.select({
      ...
      likes: sql<number>`cast(count(${promptLikes}) as int)`
    }).from(prompts).leftJoin(promptLikes, eq(promptLikes.promptId, prompts.id))
      .groupBy(prompts.category).where(eq(prompts.visibility, "Public"))
